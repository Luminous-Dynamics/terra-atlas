...
function generateUSACEDams() {
    ...
    for (const [state, count] of Object.entries(stateWeights)) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const potentialMW = hasHydro ? 0 : Math.min(50, // Cap at 50MW for small hydro
...
function analyzeRetrofitOpportunities(dams) {
    console.log('\n🔍 Analyzing retrofit opportunities...');
    const viableDams = dams.filter(d => d.retrofit_potential_mw > 0.5 &&
        d.overall_viability_score > 60 &&
        d.payback_period_years < 15);
    const byState = {};
    viableDams.forEach(dam => {
        if (!byState[dam.state])
            byState[dam.state] = [];
...
function main() {
    ...
    console.log(`  Investment Required: $${(stats.total_investment_required / 1000000000).toFixed(1)}B`);
    console.log(`  Annual Generation: ${stats.total_annual_generation_twh} TWh`);
    console.log(`  Annual Revenue: $${(stats.total_annual_revenue_potential / 1000000000).toFixed(1)}B`);
    ...
    stateOpportunities.slice(0, 5).forEach((s, i) => {
        ...
        console.log(`     Investment: $${(s.total_investment / 1000000000).toFixed(1)}B`);
