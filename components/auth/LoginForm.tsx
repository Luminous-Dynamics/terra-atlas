...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (analytics) {
        analytics.trackError('Login error', error?.toString());
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github' | 'linkedin') => {
    ...
    try {
      if (analytics) {
        analytics.trackEvent('oauth_login_attempt', {
          provider
        });
      }
      ...
      if (analytics) {
        analytics.trackEvent('oauth_login_failed', {
          provider,
        ...
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700 p-8">
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              <button
                ...
                disabled={loading}
                className="w-full inline-flex justify-center py-3 px-2 border border-gray-600 rounded-lg bg-gray-900/50 text-gray-300 hover:bg-gray-800/50 disabled:opacity-50 disabled:cursor-not-allowed transition"
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-400">
              ...
              <Link href="/auth/signup" className="text-green-400 hover:text-green-300 transition font-medium">
                Sign up
              </Link>
            </p>
          </div>
